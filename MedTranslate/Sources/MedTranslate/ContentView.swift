...
struct ContentView: View {
    ...
    private var header: some View {
        HStack(spacing: 14) {
            ...
            Button(store.recorder.isRecording ? "Arrêter et traduire" : "Démarrer l’enregistrement") { store.toggleRecording() }
...
struct SettingsView: View {
    @State private var key = Keychain.read() ?? ""
    @State private var saved = false
    var body: some View {
        Form {
            SecureField("Clé API OpenAI", text: $key)
            Text("La clé est enregistrée uniquement dans le Trousseau macOS. Elle n’est jamais ajoutée au dépôt.").font(.caption).foregroundStyle(.secondary)
            HStack { Button("Enregistrer") { try? Keychain.save(key.trimmingCharacters(in: .whitespacesAndNewlines)); saved = true }; Button("Supprimer", role: .destructive) { Keychain.delete(); key = "" }; if saved { Text("Enregistrée").foregroundStyle(.green) } }
