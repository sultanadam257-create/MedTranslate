...
final class CourseStore: ObservableObject {
    ...
    private func finishRecording() async {
        ...
        do {
            let transcript = try await SpeechService.transcribe(audioURL: audio)
            course.transcript = transcript
            course.translation = "Traduction Apple en cours…"
            course.vocabulary = MedicalGlossary.items(in: transcript)
            course.date = Date()
        } catch { errorMessage = error.localizedDescription }
    }
